...
// actually live on the `Scene` builder in `ClioApp.body`
...
// a non-executing reference, and `ClioApp.body` should match
// it byte-for-byte.
//
// Rules:
//   - Do not change chrome modifiers in `ClioApp.body` without
...
/// modifiers are applied in `ClioApp.body` (since SwiftUI
...
/// ```
///
/// If you need to change the chrome, change it in `ClioApp.body`
